"""
Agentic chat system with multi-provider support.

Supports two LLM backends, selected via the CHAT_PROVIDER environment variable:
  - "anthropic" (default) — Anthropic native tool-use API with streaming
  - "openai"              — LangGraph ReAct agent using OpenAI GPT models

Set CHAT_PROVIDER=openai in your .env to use OpenAI (requires OPENAI_API_KEY).
Anthropic is the default and recommended backend (requires ANTHROPIC_API_KEY).
"""
...
from utils.llm.clients import anthropic_client, ANTHROPIC_AGENT_MODEL, llm_agent, llm_agent_stream
...
# Provider switch
# ---------------------------------------------------------------------------
# Set CHAT_PROVIDER=openai in your .env to use the OpenAI/LangGraph backend.
# Defaults to "anthropic" — no change in behavior for existing deployments.
CHAT_PROVIDER = os.getenv('CHAT_PROVIDER', 'anthropic').lower()
logger.info(f"Chat provider: {CHAT_PROVIDER}")

# ---------------------------------------------------------------------------
# Shared tool list — identical for both providers
# ---------------------------------------------------------------------------
# IMPORTANT: Keep this list fixed and in this exact order.
# Both Anthropic and OpenAI cache tool definitions as part of the request
# prefix — changing the order breaks the cache and increases token costs.
...
def _build_system_prompt_with_app_tools(base_prompt: str, app_tools: list) -> str:
    """Append app tool awareness block to system prompt when app tools are present."""
    if not app_tools:
        return base_prompt
...
# ---------------------------------------------------------------------------
# Streaming callback — base (Anthropic path uses this directly)
# ---------------------------------------------------------------------------
...
async def _execute_agentic_chat_stream_anthropic(
    ...
    system_prompt = _build_system_prompt_with_app_tools(system_prompt, app_tools)
...
async def _execute_agentic_chat_stream_openai(
    ...
    task = asyncio.create_task(_run_openai_agent_stream(agent, lc_messages, config, callback, full_response))
...
async def execute_agentic_chat_stream(
    ...
    Provider is selected by the CHAT_PROVIDER env var (default: anthropic).
